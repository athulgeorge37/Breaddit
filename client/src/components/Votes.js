...
import { get_curr_user_vote, get_vote_count, make_vote } from '../rest_api_requests/VoteRequests';


function Votes({ vote_type, post_id=null, comment_id=null, prop_indented=false, prop_comment_id="" }) {
    ...
    const initalise_vote_counts = async () => {
        ...
        const up_vote_response = await get_vote_count(parent_id_to_use, vote_type, true)
        const down_vote_response = await get_vote_count(parent_id_to_use, vote_type, false)
    ...
    const handle_vote_change = async (new_vote) => {
        ...
        const response = await make_vote(post_id, comment_id, vote_type, vote_to_send);
        console.log(response)
        if (response.error) {
            return
        }
        ...
        // updating the up and down vote counters to reflect changes in DB
        if (new_vote === true) {
        ...
        <div className="votes">
            ...
            >
                ...
                <img 
                    ...
                    alt="up_vote" 
                    className={`vote_img up_vote ${curr_user_vote === true ? "active" : ""}`}
                />
            </button>
            ...
            >
                ...
                <img 
                    src="./images/up_arrow_v3.png" 
                    alt="up_vote" 
                    className={`vote_img down down_vote ${curr_user_vote === false ? "active" : ""}`}
                />
            </button>
