...
function PaginatedTableRow({ user_data }) {
    ...
    return (
        <tr
            ...
            onClick={() => {
                navigate(
                    `/admin_dashboard/user_overview/${user_data.id}`,
                    data
                );
            }}
        >
            ...
            <td className="role item">{user_data.role}</td>
            <td className="username item">{user_data.username}</td>
            <td className="email item">{user_data.email}</td>
            <td className="date_joined item">
                {get_current_date(user_data.createdAt)}
            </td>
            <td className="is_banned item">
                <button className={profile_stats.is_banned ? "banned" : ""}>
                    {profile_stats.is_banned ? "Banned" : "Permitted"}
                </button>
            </td>
            <td className="follower item">{profile_stats.follower_count}</td>
            <td className="following item">{profile_stats.following_count}</td>
            <td className="no_of_posts item">{profile_stats.no_of_posts}</td>
            <td
                className={`is_online item ${
                    profile_stats.is_online ? "active" : ""
                }`}
            >
