...
function Profile() {
    ...
    const { data, refetch } = useQuery(["profile_visit_query"], () => {
        if (current_user.role === "user" && username_route !== current_user.username) {
            return make_profile_visit(get_last_minute(), new Date(), username_route)
        } else {
            return null
        }
    }, {
