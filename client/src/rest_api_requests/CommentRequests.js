...
const create_comment_or_reply = async (post_id, text, type, parent_comment_id=null) => {
    // parent_comment_id is not required when making comments,
    // only required for replies

    const response = await axios.post(`${CUSTOM_ENDPOINT}/create/of_type/${type}`, {
        post_id: post_id,
        text: text,
...
const delete_comment_or_reply = async (type, id) => {
    ...
    const response = await axios.delete(`${CUSTOM_ENDPOINT}/delete/of_type/${type}/by_id/${id}`, {
        headers: {
            web_access_token: get_item_local_storage("web_access_token")
        }
    })
...
const edit_comment_or_reply = async (comment_id, updated_text) => {
    const response = await axios.put(`${CUSTOM_ENDPOINT}/edit_comment_or_reply`, {
