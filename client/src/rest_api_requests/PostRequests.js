...
const create_post = async (title, text, image) => {
    const response = await axios.post(`${CUSTOM_ENDPOINT}/create_post`, {
        title: title,
        text: text,
        image: image,
        edited: false
...
const delete_post = async (post_id) => {
    const response = await axios.delete(`${CUSTOM_ENDPOINT}/delete_post/by_id/${post_id}`, {
        headers: {
            web_access_token: get_item_local_storage("web_access_token")
        }
    })
...
const edit_post = async (post_id, post_title, post_text, post_image) => {
    const response = await axios.put(`${CUSTOM_ENDPOINT}/edit_post`, {
        post_id: post_id,
...
const get_all_posts_by_curr_user = async () => {
    const response = await axios.get(`${CUSTOM_ENDPOINT}/get_all/by_curr_user`, {
        headers: {
            web_access_token: get_item_local_storage("web_access_token")
        }
    });
